...
router.post('/postData', (req, res) => {
    ...
    const gender = req.body.data.data[4];
    const email = req.body.data.data[5];
    const tel = req.body.data.data[6];
    ...
    db.query("insert into test(name, age, test_data, address, gender, email, tel) values(?, ?, ?, ?, ?, ?, ?)", [name, age, add, address, gender, email, tel], (err, rows) => {
        if(!err) {
            res.send(success);
            console.log('success')
        } else {
